...
export default function CorrectionPage() {
  ...
  useEffect(() => {
    if (selectedUserId) {
      loadPredictionsForUser(selectedUserId);
    }
  }, [selectedUserId]);
  ...
  async function loadInitialData() {
    ...
    if (adminProfileError) {
      setMessage(`Erreur profil admin : ${adminProfileError.message}`);
      setLoading(false);
      return;
    }

    if (!adminProfile?.is_admin) {
      setMessage('Accès refusé : cette page est réservée aux admins.');
      setLoading(false);
      return;
    }
    ...
    setSelectedDate((currentDate) => {
      ...
      const stillHasMatchesOnCurrentDate = loadedMatches.some((match) => {
        return match.status !== 'finished' && toDateKey(new Date(match.kickoff_at)) === currentDateKey;
      });
  ...
  async function loadPredictionsForUser(userId: string) {
    setMessage('');
    ...
    if (error) {
      setMessage(`Erreur pronostics : ${error.message}`);
      return;
    }
    ...
    setPredictions((current) => {
      ...
      if (field === 'predicted_first_scorer_id') {
        ...
        const player =
          ...
          getMatchPlayers(match).find((p) => p.id === playerId);
  ...
  async function save(match: Match) {
    if (!selectedUserId) {
      return setMessage('Sélectionne un utilisateur.');
    }
    ...
    if (
      ...
      return setMessage(
        `Cet utilisateur a déjà utilisé son bonus x2 pour ${
    ...
    if (error) {
      ...
      setMessage('Correction sauvegardée.');
      await loadPredictionsForUser(selectedUserId);
    }
  }
  ...
  function renderMatchCard(match: Match, mode: ViewMode) {
    ...
    return (
      <div key={match.id} style={{ display: 'grid', gap: 8 }}>
        <div className={`card prediction-card ${cardStateClass}`}>
          <div className="points-pill">
            {livePreview ? `${livePreview.points} pts live` : `${p?.points ?? 0} pts`}
          </div>

          <div className="prediction-badges">
            {!p && <span className="badge pending">🟡 À corriger</span>}
            {p && <span className="badge saved">✅ Prono enregistré</span>}
            {match.status === 'finished' && <span className="badge finished">🏁 Terminé</span>}
            {match.status === 'live' && <span className="badge fire">🔴 Live</span>}
          ...
          >
            <button onClick={() => save(match)}>
              {p ? 'Mettre à jour la correction' : 'Sauvegarder la correction'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <main className="container">
      <h1>Correction des pronostics</h1>
      ...
      {!isAdmin && message && <p className="error">{message}</p>}

      {isAdmin && (
        <>
          <div className="card">
            <h2>Utilisateur à corriger</h2>
            ...
            <p className="small" style={{ marginTop: 10 }}>
              Les corrections seront enregistrées pour cet utilisateur.
            </p>
          </div>
