...
const awardLabels: Record<AwardKey, string> = {
  ...
  top_assist_id: 'Meilleur passeur',
  ...
  async function load() {
    ...
    const [
      ...
      supabase
        ...
        .select(
          'id, winner_team_id, best_player_id, top_scorer_id, top_assist_id, best_goalkeeper_id, updated_at'
        )
        .eq('id', 1)
        .maybeSingle(),
    ...
    if (settingsData) {
      ...
      setValues({
        ...
        top_assist_id: settingsData.top_assist_id || '',
    ...
    const { error } = await supabase.from('tournament_settings').upsert(
      {
        ...
        top_assist_id: values.top_assist_id || null,
      ...
      {!loading && (
        <>
          <div className="grid">
            ...
            {renderAwardSelector('top_assist_id')}
